...
- [ ] 19.1 实现可选时间模块的 Definition、Command、Event、Reducer、Tool 和投影，时间推进必须显式且可审计
- [ ] 19.2 实现可选活动模块，支持耗时、资源代价、前置条件、成功/失败结果和中断策略
- [ ] 19.3 实现可选旅行模块，使用地点 Definition 与连接关系表达出发、到达、耗时、风险和场景切换
- [ ] 19.4 实现基于世界时间与已提交事件的计划触发器，不读取系统时钟决定游戏事实
- [ ] 19.5 将所有随机结果写入 Random Record，并验证恢复和重放不重新掷骰
- [ ] 19.6 为等待、休息、搜寻、治疗和旅行配置题材无关示例，并通过 PresentationDefinition 显示结果
- [ ] 19.7 添加边界、并发、失败推进、跨日触发、随机审计和两个契约世界确定性测试
