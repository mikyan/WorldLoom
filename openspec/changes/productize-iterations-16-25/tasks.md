...
- [x] 23.1 为内置 `war-survival` 固定内容版本、模块清单、角色创建 Profile、初始场景、目标与结束条件
- [x] 23.2 编写约 45–90 分钟的完整短冒险，包含 7–14 个关键场景或阶段、至少 2 名可交互 NPC 和至少 3 条有意义的行动路线
- [x] 23.3 配置地点、旅行、活动、物品、资源、状态、关系、任务、进度钟与 Behavior，使剧情推进完全来自声明式内容和通用模块
- [x] 23.4 配置至少 3 个可达结局以及失败后的替代推进路径，避免单次失败导致无反馈死局
- [x] 23.5 完成 PresentationDefinition、场景叙述素材、可选行动、状态摘要和结局总结，不在通用 UI 中读取世界私有键
- [x] 23.6 完成 NPC 知识、秘密、目标、关系与感知边界配置，并验证未揭示事实不进入玩家上下文或公开重放
- [x] 23.7 建立成功、代价成功和失败结局的黄金路线，以及资源耗尽、非法行动、存档恢复和确定性重放测试
- [x] 23.8 将该世界设为内置试玩入口，同时运行 `station-ai` 契约以确认新增能力没有引入战争题材分支

## 24. 游玩界面、存档与重放

- [x] 24.1 调整共享 UI 信息架构，围绕开始游戏、继续游戏、当前场景、叙事时间线、行动和角色/目标状态组织页面
- [x] 24.2 实现 Run 目录投影与 `SaveCoordinator`，支持创建、列出、重命名、归档、继续和完成多个内置世界 Run
- [x] 24.3 在继续游戏前校验固定世界版本、事件连续性、Snapshot 兼容和 Reducer 重建，并提供坏 Snapshot 回退与 EventLog 损坏诊断
- [x] 24.4 实现虚拟化叙事时间线和事件详情投影，关联行动、判定、Random Record、Behavior 因果、NPC 公开动作和玩家可见叙述
- [x] 24.5 实现 Definition 驱动的角色状态、库存、关系、任务和进度钟卡片，并支持窄屏、桌面与减少动态效果
- [x] 24.6 提供应用内公开重放与离线一致性校验，默认排除密钥、模型正文、NPC 私有记忆和未揭示秘密
- [x] 24.7 添加长时间线分页、跨 Run 隔离、篡改检测、恢复后继续游玩、隐私和三端 UI/性能防回归测试
