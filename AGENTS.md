...
根据用户反馈迭代时，只保留**当前正确的目标、事实和设计选择**，不要把讨论过程中的错误方案、纠正过程或方法论本身写进最终设计。

例如：

* 原文是“A 是 B”，用户纠正为“A 是 C”，直接改成“A 是 C”，不要再补充“A 不是 B”。
* 用户要求“先推理，再建立概念”，就直接按这种方式设计，不要把“先推理再建立概念”本身变成新的设计概念或文档内容。
...
**曾经怎么想 → 为什么错 → 用户如何纠正 → 以后不要这样做**

讨论过程中产生的方法论、禁忌和纠偏经验，如需长期保留，应记录在独立的**影子笔记**中，不污染正式设计文档。

影子笔记与原文档放在同一目录，命名规则为：

**`<原文件名>-shadow.<扩展名>`**

例如：

* `foo.md` → `foo-shadow.md`
* `architecture.md` → `architecture-shadow.md`

后续修改正式文档时，应同步检查并更新对应的影子笔记。
