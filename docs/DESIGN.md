...
启用 `worldloom.rules.scene-exploration` 的世界使用事件溯源节点图建立方向感。世界包声明题材无关的 Node、Connection、Affordance、SceneFrame 与独立 GuidanceSuggestion；玩家完全未知的条目不进入 Presentation。进入场景、行动结算、旅行到达或获准 NPC 知识揭示会在同一个原子 EventLog 批次追加 `ExplorationKnowledgeRevealedEvent`，Reducer 只更新可选模块状态，地图、主持上下文和 UI 都从该状态投影，不能把模型正文当作地图事实。节点公开状态为 `RUMORED`、`DISCOVERED`、`BLOCKED` 或 `VISITED`，普通场景切换不会让知识倒退。

每个探索场景提供至多三句感官框架、当前目标、公开压力、已知对象/出口和开放式收尾问题。桌面端在聊天左侧按需打开可关闭场景抽屉，右侧紧凑角色栏宽度不变；移动端从安全区场景条打开全屏地图，地图关闭前聊天和输入框保持挂载，因此滚动位置与草稿不会丢失。建议不再由全部 Action/Activity/Travel 自动生成，而必须单独创作并绑定当前可见依据；点击只预填可编辑自然语言。新手、标准、沉浸三档引导和主动“需要提示”属于客户端偏好，不写 EventLog，也不替玩家执行动作。
...
共享页面的颜色角色、字阶、间距、响应式骨架、组件语义和视觉验收以[游戏页面风格规范](GAME_UI_STYLE_GUIDE.md)为准。世界包可以提供经验证的背景、头像和公开文案，但不能覆盖共享语义色、可访问性规则或提交任意 UI 代码。
...
当前仓库已完成三十五轮工程基线、发布 `0.0.3` 并进入 `0.0.4` 候选开发，包含 KMP/Compose 工程骨架、Definition 与 TypedValue、Command/Event、Reducer、回放、application session、共享 UI，以及 Android、iOS 和 Desktop 平台入口。
...
世界可以显式启用 `world-time`、`activity` 与 `travel` 模块。世界时间以分钟整数保存在模块状态中，只能由类型化 Command 推进；等待、活动耗时、旅行耗时和到期计划全部生成版本化 Event，不读取系统时钟裁决游戏事实。活动通过场景列表表达前置条件，通过结果 Effect 表达成本与收益，并可配置独立的中断结果和已消耗时间；旅行通过有向路线表达起点、终点、风险结果、额外耗时、受阻或抵达后的场景切换。跨越计划时刻时，触发事件与效果和本次行动作为一个原子批次提交，已触发 ID 随世界时间投影保存，因此恢复和重放不会重复触发。

主持人可用的 `time.advance`、`activity.perform` 与 `travel.perform` Tool Schema 只列出当前场景有效选项。需要检定的活动或旅行仍先写入 `CheckResolvedEvent` 与 `RandomRecord`，再与时间和效果事件原子追加；如果追加失败，Runtime 根据已提交随机记录恢复随机服务位置，重试不会因为未落盘的抽取而换一个结果。共享 UI 与 Presentation 只显示世界时间、可用活动、路线和事件摘要，不读取世界私有字段。
...
`playable-world/v1` 可声明稳定 NPC Profile，把 Entity、显示名、身份提示、唤醒 Event 类型、可见 Presentation、目标、私有知识和公开动作白名单绑定在一起。`NpcContextProjector` 只投影该 NPC 当前参与的场景、明确许可字段以及自己的私有记忆；其他 NPC 的秘密、模型正文和场景外事件不会进入上下文。初始场景参与者也通过角色创建 Event 成为可回放事实，不从世界包旁路读取当前状态。

主持工具提交事件后，`NpcSceneOrchestrator` 同步补扫 EventLog，以 `EventId + NpcId` 创建稳定 SQLDelight 工作项并按事件序列、NPC ID 排序。全量补扫覆盖事件已提交但工作项尚未创建的终止窗口；遗留 `RUNNING` 项标为中断，不重新调用模型。每事件唤醒数、每次调度 Turn 数和场景并发受限，每个 NPC Turn 继续使用 Agent Runtime 的步骤、工具、Token、费用与超时预算。前台主持回合持有 Provider 优先级，NPC 不启动无主后台请求。

NPC 模型只能通过 `npc.speak` 或 `npc.act` 发布玩家可见内容；Tool Gateway 根据 Actor、当前场景参与者和动作白名单重新校验，`PublishNpcActionCommand` 再由 WorldEngine 生成公开 Event。模型最终回复、信念与记忆保存在独立私有分区，主持 Agent 和 Presentation 只聚合显式公开 Event。NPC 失败、预算耗尽或工具拒绝不会撤销玩家行动和已经提交的世界事实。

内置 `war-survival` 内容 v1 是约 60 分钟的可重复试玩竖切《灰烬中的车队》。世界包声明 10 个关键场景、14 个场景行动、2 名 NPC、时间/活动/旅行、库存/状态/关系/任务/进度钟、3 个 Behavior 和 3 个可达结局；失败结果进入替代场景或失败结局，不依赖 Runtime 题材分支。`catalogPriority` 只提供题材无关的目录排序，`contentVersion` 固定内容兼容边界。主持人上下文读取公开场景描述和当前动态行动，NPC 仍只读取自己的感知与秘密；完成 Run 保留最终 Presentation、时间线、状态和回放入口。

内置 `station-ai` 内容 v2 是第二个约 60 分钟的完整试玩《静默轨道：赫利俄斯危机》。世界包声明 9 个场景、17 个关键行动、8 个目标、莱拉与索伦两名 NPC、全部现有通用冒险模块、3 个 Behavior 和成功/代价成功/失败 3 个可达结局。检定或选择失败会推进到隔离、救生艇或明确失败结局，不以重试死循环掩盖内容缺口。三条黄金路线通过相同 application session 写入 SQLDelight EventLog/Snapshot，随后验证内容版本恢复、公开回放和完整最终投影；NPC 对话与固定知识摘要继续遵守当前场景、角色白名单和独立记忆边界。
...
第 33 轮把 `station-ai` 从跨题材契约夹具扩展为第二个完整内置短篇。内容 v2 用同一 `playable-world/v1`、Module Registry、主持动态 Tool、NPC 调度、SQLDelight EventLog/Snapshot 与公开回放运行 9 个场景和三条结局路线。持久 EventStore 初始化时同时固定世界内容版本，避免直接创建 Session 时把 v2 Run 错记为 v1；恢复仍拒绝内容版本不匹配。世界包测试与题材审计确保新增内容没有在 Runtime、Presenter 或存档层引入 `station.*` 分支。

第 34 轮把 TXT/EPUB 输入收敛到独立识别工作区。`RecognitionJobState` v1 用 SHA-256 绑定来源字节，并以 revision、阶段、进度、诊断和草稿版本写入 SQLDelight；恢复必须重新核对来源身份，变化后只能创建新 JobId。解析和识别在工作 Dispatcher 中执行，协程取消传播后清除临时正文与未发布草稿。角色、地点、场景、目标和候选事实都引用 SourceChunk、合法字符范围与置信说明；共享 UI 只读展示候选、诊断和有界来源上下文。完成识别本身不授权写入内置目录或正式 Run，只有通过下一段聚合门禁的固定草稿版本才能试玩或安装。

第 35 轮以 `DraftPlayabilityValidator` 统一手写世界与来源草稿的准入条件：先由 WorldPackage Loader 检查容器、Definition、模块、引用、`playable-world/v1` 和 Behavior Command 白名单，再要求黄金路线完整模拟并覆盖每个结局；带 Recognition Job 的草稿还必须使用完成检查点，并让每个候选引用合法 SourceChunk 范围与置信诊断。任一错误在创建 Run 或调用 Provider 前返回具体路径。`HostedDraftSandboxManager` 只把 manifest、Definition、角色 Profile、可玩契约和已验证 Behavior 装入 `sandbox.*` 命名空间，复用正式建角、主持、NPC、Behavior、EventLog/Reducer、存档投影和公开回放，但为 Event、GM/NPC Session、Turn、Work 与 Memory 分配独立 Store；重置和删除不接触正式目录。安装会剥离来源、中间生成数据、Agent 数据与沙箱事实，对清理包再次验证、计算 SHA-256 内容地址并以期望旧地址原子发布，失败保持上一版本。`round35CandidateGate` 聚合完整检查、迁移、隐私/题材审计、Android/Desktop Release、哈希、Desktop 启动存活和 iOS Simulator Kotlin 编译；在真机与真人证据缺失时仍保持 candidate。

平台凭据边界已落地 Android Keystore、iOS Keychain 与 Windows 用户级 DPAPI；非 Windows 的 Desktop 目前使用仅会话内存回退。共享 Runtime 已支持安全路径、CRC、大小限制和重复项检查的 `.worldloom` v1 STORED ZIP 容器、版本化 Behavior AST/Command 白名单、四种角色创建模式、RuleProfile，以及 Brief/TXT/EPUB 到可加载世界包的分阶段生成。识别工作区任务已经持久化，既有 `GenerationTaskStore` 和 Sandbox Store 仍只有内存实现。Desktop 与 Android 的 EPUB 平台读取支持压缩条目和 GB18030；iOS 公共生成逻辑已编译，平台文件选择与压缩 EPUB/GB18030 桥接仍待接入。包签名、Anthropic Adapter、持久化生成任务/沙箱实现和语音仍属于后续增量。

工程初始化的范围见[项目初始化设计](PROJECT_INITIALIZATION.md)，逐轮实现与验收证据见[迭代执行记录](ITERATION_EXECUTION.md)，Alpha 门禁见[封闭 Alpha 验收](ALPHA_ACCEPTANCE.md)，内置试玩内容见[战争生存短剧本说明](BUILT_IN_WAR_SCENARIO.md)与[空间站短剧本说明](BUILT_IN_STATION_SCENARIO.md)，存档与回放边界见[存档与公开回放说明](SAVES_AND_PUBLIC_REPLAY.md)，来源任务边界见[TXT/EPUB 剧本识别工作区](SOURCE_RECOGNITION_WORKSPACE.md)，草稿验证、隔离和安装边界见[世界草稿试玩沙箱](DRAFT_PLAYTEST_SANDBOX.md)。

从第十六轮开始，产品验收顺序调整为“先证明人工编写的内置世界可从开局玩到结局，再扩展识别和自动生成”。第十七至二十二轮依次补齐建角、主持人、时间与冒险状态、Behavior 和 NPC，第二十三轮完成内置短剧本，第二十四轮完成多 Run 存档与公开回放，第二十五轮完成封闭 Alpha 工程门禁，第二十六轮开始深化可恢复主持体验。第 26–33 轮继续处理主持历史、NPC 对话、长局连续性、引导、恢复和第二内置世界；第 34 轮完成 TXT/EPUB 识别工作区，第 35 轮接入聚合验证、隔离草稿试玩和原子安装。`playable-world/v1` 是后续生成器必须满足的可玩性目标，而不是由生成流程反向决定 Runtime 结构。
...
探索定义是向后兼容的可选扩展：未声明 `exploration` 的旧包不生成地图或发现事实；声明后必须在 manifest 和 `requiredModuleIds` 中启用探索模块。校验器要求知识 ID 唯一、连接端点闭合、场景框架完整、Reveal 引用合法、核心线索在入场或所有失败推进结果中可得，并要求每条建议的依据当前能够公开、动态目标属于对应场景。未来版本的探索 Schema 必须明确拒绝，不能静默按 v1 解释。
...
2. 用内置世界实现 `GameTurnOrchestrator`、主持人可见上下文、受限工具循环和事实一致叙述；
3. 完成 Definition、TypedValue、动态组件、模块注册和 PresentationDefinition 的基础 Schema；
4. 验证均匀分布的 300～1000 日战争在极短与极长存档中的节奏、内容密度和结局质量；
5. 在真实创作样本上扩充 `CharacterCreationProfile` 与 `RuleProfile` 的可组合原语目录；
6. 为 iOS 接入 TXT/EPUB 平台文件桥，并验证压缩 EPUB 与 GB18030；
7. 增加 Behavior 状态机/行为图编译和 DSL 版本迁移；
8. 用真实模型和长对话调优 50%/75% 水位线、CompactionModel、召回数量和记忆衰减权重；
9. 为内容生成任务接入持久化 Store，验证进程崩溃后的继续执行；
10. 在 Android、iOS 与 Windows 最低基线设备上验证帧时间、常驻内存、长存档和后台任务竞争。
