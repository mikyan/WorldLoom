...
当前仓库已完成三十四轮工程基线并处于 `0.1.0-alpha.1` 候选，包含 KMP/Compose 工程骨架、Definition 与 TypedValue、Command/Event、Reducer、回放、application session、共享 UI，以及 Android、iOS 和 Desktop 平台入口。
...
第 34 轮把 TXT/EPUB 输入收敛到独立识别工作区。`RecognitionJobState` v1 用 SHA-256 绑定来源字节，并以 revision、阶段、进度、诊断和草稿版本写入 SQLDelight；恢复必须重新核对来源身份，变化后只能创建新 JobId。解析和识别在工作 Dispatcher 中执行，协程取消传播后清除临时正文与未发布草稿。角色、地点、场景、目标和候选事实都引用 SourceChunk、合法字符范围与置信说明；共享 UI 只读展示候选、诊断和有界来源上下文。识别草稿不进入内置目录或正式 Run，第 35 轮聚合验证与隔离沙箱完成前不能试玩或安装。

平台凭据边界已落地 Android Keystore、iOS Keychain 与 Windows 用户级 DPAPI；非 Windows 的 Desktop 目前使用仅会话内存回退。共享 Runtime 已支持安全路径、CRC、大小限制和重复项检查的 `.worldloom` v1 STORED ZIP 容器、版本化 Behavior AST/Command 白名单、四种角色创建模式、RuleProfile，以及 Brief/TXT/EPUB 到可加载世界包的分阶段生成。识别工作区任务已经持久化，既有 `GenerationTaskStore` 仍只有内存实现。Desktop 与 Android 的 EPUB 平台读取支持压缩条目和 GB18030；iOS 公共生成逻辑已编译，平台文件选择与压缩 EPUB/GB18030 桥接仍待接入。包签名、Anthropic Adapter、持久化生成任务实现和语音仍属于后续增量。

工程初始化的范围见[项目初始化设计](PROJECT_INITIALIZATION.md)，逐轮实现与验收证据见[迭代执行记录](ITERATION_EXECUTION.md)，Alpha 门禁见[封闭 Alpha 验收](ALPHA_ACCEPTANCE.md)，内置试玩内容见[战争生存短剧本说明](BUILT_IN_WAR_SCENARIO.md)与[空间站短剧本说明](BUILT_IN_STATION_SCENARIO.md)，存档与回放边界见[存档与公开回放说明](SAVES_AND_PUBLIC_REPLAY.md)，来源任务边界见[TXT/EPUB 剧本识别工作区](SOURCE_RECOGNITION_WORKSPACE.md)。

从第十六轮开始，产品验收顺序调整为“先证明人工编写的内置世界可从开局玩到结局，再扩展识别和自动生成”。第十七至二十二轮依次补齐建角、主持人、时间与冒险状态、Behavior 和 NPC，第二十三轮完成内置短剧本，第二十四轮完成多 Run 存档与公开回放，第二十五轮完成封闭 Alpha 工程门禁，第二十六轮开始深化可恢复主持体验。第 26–33 轮继续处理主持历史、NPC 对话、长局连续性、引导、恢复和第二内置世界；第 34 轮完成 TXT/EPUB 识别工作区，第 35 轮再接入隔离草稿试玩。`playable-world/v1` 是后续生成器必须满足的可玩性目标，而不是由生成流程反向决定 Runtime 结构。
